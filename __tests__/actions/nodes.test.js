  ...
  describe('transactions', () => {
    ...
    describe('remove', () => {
      ...
      it('actions', () => {
        ...
        expect(actions[1].type).toBe('UNSET_EXPANDED_DEEP')
        expect(actions[1].data.treePath).toEqual([uuid, uuid1])
        return actions[actions.length - 1].then(() => {
          const lastAction = store.getActions().pop()
          expect(lastAction.type).toBe('UPDATE_NODE_TRANSACTION_STATUS')
          expect(lastAction.data.status).toBe('COMMITTED')
    ...
    describe('move', () => {
      ...
      it('actions', () => {
        ...
        expect(actions[2].type).toBe('UNSET_EXPANDED_DEEP')
        expect(actions[2].data.treePath).toEqual([uuid, uuid2])
        expect(actions[3].type).toBe('SET_UI_KEY')
        expect(actions[3].data.key).toBe('active')
        expect(actions[3].data.treePath).toEqual([uuid1, uuid2])
        return actions[actions.length - 1].then(() => {
          const lastAction = store.getActions().pop()
          expect(lastAction.type).toBe('UPDATE_NODE_TRANSACTION_STATUS')
          expect(lastAction.data.status).toBe('COMMITTED')
